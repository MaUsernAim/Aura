...
func main() {
        ...
        for i := 0; i < len(trackMatches) && i < len(mvMatches); i++ {
                ...
                energyResult := finalResult * variance * 1000.0

                // Добавляем запись в слайс (убедись, что поля Variance и EnergyResult добавлены в твою структуру TrackResult)
                results = append(results, TrackResult{
                        ...
                        Variance:  variance,
                        Result:    finalResult,
                        Energy:    energyResult, // или EnergyResult в зависимости от имени поля в структуре
                })
        }
	...
	fmt.Printf("%-10s | %-12s | %-12s | %s\n", "№ Трека", "Asymmetry", "Total Mode", "Результат: (2+Asym)*Mode")
	fmt.Println("---------------------------------------------------------------------")

	// Вывод отсортированных данных
	for _, track := range results {
		fmt.Printf("Трек #%-3s | %-12.3f | %-12.3f | %.4f\n", track.Number, track.Asymmetry, track.Mode, track.Result)
	}
}
